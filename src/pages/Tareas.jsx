...
// NUEVO: Importamos 'where' para filtrar por usuario
...
const Tareas = () => {
  ...
  // Obtener inicial del usuario para el perfil
  ...
  // --- LECTURA AISLADA POR USUARIO ---
  useEffect(() => {
    const unsubscribeAuth = auth.onAuthStateChanged((user) => {
      if (user) {
        const tareasRef = collection(db, 'tareas');
        
        // NUEVO: query con 'where' para buscar solo las de este user.uid
        const q = query(
          tareasRef, 
        ...
        const unsubscribeFirestore = onSnapshot(q, (snapshot) => {
          ...
          // Capturamos el error de Firebase por si pide crear el Índice
          console.error("Error al traer tareas:", error.message);
        });
        
        return () => unsubscribeFirestore();
      } else {
        setTareas([]);
        navigate('/login'); // Expulsar si no hay usuario
      }
    });
    ...
    try {
      if (tareaEditando.esNueva) {
        ...
        await addDoc(collection(db, 'tareas'), {
          ...
          // NUEVO: Guardamos el ID del creador
    ...
    <div style={{ ...styles.appContainer, backgroundImage: `url(${bgImage})` }}>
      ...
      <header style={styles.header}>
        ...
        <div style={styles.headerRight}>
           <div style={styles.profileIcon}>{inicialUsuario}</div>
        </div>
      </header>
...
const styles = {
  ...
  cardIdBadge: { padding: '2px 6px', borderRadius: '10px', fontSize: '10px', fontWeight: 'bold', fontFamily: 'monospace', borderStyle: 'solid', borderWidth: '1px' },
  cardHeaderRight: { display: 'flex', alignItems: 'center', gap: '8px' },
  typeTagText: { fontSize: '9px', fontWeight: 'bold', padding: '2px 6px', borderRadius: '4px', border: '1px solid' },
  starIcon: { fontSize: '16px', cursor: 'pointer', userSelect: 'none', transition: 'transform 0.2s' },
  cardTitle: { fontSize: '12px', color: '#172B4D', fontWeight: '500', lineHeight: '1.4', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden', textOverflow: 'ellipsis' },
